...
- 1. Réponse du serveur DNS (adresse IP)

Ensuite, le navigateur contacte le serveur à cette adresse IP.

- 1. Envoi d’une **requête HTTP**, avec une méthode (GET/POST)
- 1. Réception d’une **réponse HTTP** (avec statut, en-têtes, contenu)
- 1. Affichage par le navigateur du contenu reçu.
