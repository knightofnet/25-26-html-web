...
- [Bilan TD1 + TD2 — Ce que vous devez maîtriser](#bilan-td1--td2--ce-que-vous-devez-ma%C3%AEtriser)
    - [Modèle client/serveur : ce qui se passe quand je tape une URL](#mod%C3%A8le-clientserveur--ce-qui-se-passe-quand-je-tape-une-url)
        - [Schéma simple des étapes](#sch%C3%A9ma-simple-des-%C3%A9tapes)
        - [HTTP : requête, réponse, codes de statut, en-têtes](#http--requ%C3%AAte-r%C3%A9ponse-codes-de-statut-en-t%C3%AAtes)
            - [Codes de statut à reconnaître](#codes-de-statut-%C3%A0-reconna%C3%AEtre)
            - [Quelques éléments de l'en-têtes de la réponse à connaître](#quelques-%C3%A9l%C3%A9ments-de-len-t%C3%AAtes-de-la-r%C3%A9ponse-%C3%A0-conna%C3%AEtre)
        - [Savoir observer une requête dans les outils de développement du navigateur](#savoir-observer-une-requ%C3%AAte-dans-les-outils-de-d%C3%A9veloppement-du-navigateur)
        - [À savoir faire procédure](#%C3%80-savoir-faire-proc%C3%A9dure)
        - [Serveur vs fichier local](#serveur-vs-fichier-local)
...
- Le départ : Vous tapez une **URL**. Exemple : `http://knightof.net/fac/htmlBase.html`

Résolution DNS : le navigateur demande au serveur DNS l’adresse IP correspondant au nom de domaine (ici `knightof.net`).

- (1) Résolution **DNS** (nom de domaine)
- (2) Réponse du serveur DNS (`knightof.net` est à l’adresse IP `123.45.67.89` [exemple fictif])

Ensuite, le navigateur contacte le serveur à cette adresse IP.

- (3) Envoi d’une **requête HTTP**, avec une méthode (GET ou POST)

A ce stade, le serveur traite la requête (cherche le fichier, exécute un script, etc.) et prépare une réponse; il peut y avoir plusieurs étapes internes côté serveur, avec une utilisation d'un langage serveur (PHP, Python, etc.) et une base de données (MySQL, etc.).
...
- En-têtes = infos utiles sur la requête et la réponse.
