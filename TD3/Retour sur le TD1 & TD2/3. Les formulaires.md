...
- [Bilan TD1 + TD2 — Ce que vous devez maîtriser](#bilan-td1--td2--ce-que-vous-devez-ma%C3%AEtriser)
    - [Les formulaires : construire, envoyer des données, GET vs POST](#les-formulaires--construire-envoyer-des-donn%C3%A9es-get-vs-post)
        - [Exemples : formulaire simple GET](#exemples--formulaire-simple-get)
        - [Exemple : formulaire simple POST](#exemple--formulaire-simple-post)
        - [À savoir vérifier dans l'onglet Réseau des outils de développement du navigateur](#%C3%80-savoir-v%C3%A9rifier-dans-longlet-r%C3%A9seau-des-outils-de-d%C3%A9veloppement-du-navigateur)
        - [Champs d'un formulaire : types courants](#champs-dun-formulaire--types-courants)
        - [Validation côté client formulaires : empêcher les erreurs simples avant envoi](#validation-c%C3%B4t%C3%A9-client-formulaires--emp%C3%AAcher-les-erreurs-simples-avant-envoi)
...
<form action="inscription.php" method="post">
  ...
  <input id="mdp" name="password" type="password" required />
...
- `<input>` : champ de saisie.
  ...
  - attribut `required` : indique que ces champs sont obligatoires.
- `<button>` : bouton pour soumettre le formulaire.
...
> Ne mettez jamais un vrai mot de passe dans une capture/partage.
...
- `<input type="checkbox" />` : case à cocher (sélection multiple possible). Pour grouper des cases à cocher, elles peuvent partager le même attribut `name` si elles font partie d’un même groupe; on ajoutera souvent `[]` à la fin du nom pour indiquer qu’il s’agit d’un tableau de valeurs (ex: `name="interests[]"`), ceci afin que coté serveur, nottamment en PHP, on puisse récupérer toutes les valeurs sélectionnées sous forme de tableau.

  ```HTML
    <label><input type="checkbox" name="interests[]" value="sports" /> Sports</label>
    <label><input type="checkbox" name="interests[]" value="music" /> Musique</label>
    <label><input type="checkbox" name="interests[]" value="movies" /> Films</label>
  ```

  Ici, si l’utilisateur coche “Sports” et “Films”, les données envoyées seront `interests[]=sports&interests[]=movies`. Coté serveur, notamment en PHP, on pourra récupérer ces valeurs sous forme de tableau :

  ```PHP
  $interests = $_POST['interests']; // $interests sera un tableau avec les valeurs cochées
  ```
