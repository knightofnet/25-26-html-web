...
function selectIncorrect($db)
{


	try {
		// Requête erronée : la table n'existe pas
		// --> C'est l'objectif de cette méthode : provoquer une erreur de requête 
		// 	   afin de tester la gestion des erreurs
		$req = "SELECT * FROM TableInvalide";
...
//code principal
debutHtml('Requête erronée');
$db = connexion();
$res = selectIncorrect($db);
