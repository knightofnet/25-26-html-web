...
function afficheJoueursP($res)
{
	...
	while ($res->fetch(PDO::FETCH_BOUND)) {
		echo "$idPersonne $nomPersonne<br>";
	}
}
...
// On choisit d'utiliser la méthode afficheJoueursP() qui utilise la méthode bindColumn() 
// de l'objet PDOStatement pour faire le lien entre les champs de la table 'personne' et 
// des variables PHP, et la méthode fetch(PDO::FETCH_BOUND) pour récupérer les données.
// On aurait aussi pu utiliser la méthode afficheJoueurs() qui utilise la méthode fetch() pour récupérer les données.
// Il y a plusieurs façons de faire la même chose en PHP, et c'est à vous de choisir 
// celle que vous préférez en fonction de vos préférences et de votre style de programmation.
afficheJoueursP($res);
