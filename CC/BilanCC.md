...
Il faillait donc obtenir, dans un premier temps, une page HTML structurée de manière à ce que les éléments soient présents, et dans le même ordre que la capture d'écran. Cet ordre est important, car c'est sur lui ensuite qu'il fallait ajouter une feuille de style CSS pour obtenir le rendu final.
...
La balise `<form>`comporte obligatoirement :
...
Voici quelque erreurs fréquentes dans la réalisation de ce formulaire, notamment :
...
Comment savoir quels titres utiliser pour chaque élément de la page ? Il fallait se référer à la capture d'écran, et observre :
...
- **Sélecteur de classe** : cible tous les éléments qui ont une classe spécifique. Par exemple, `.highlight` cible tous les éléments qui ont la classe "highlight".
  ...
  Coté HTML, on ajoute une classe à un élément en utilisant l'attribut `class`. Par exemple :
...
Si on regarde le style de bordure du titre de niveau, il est donc en partie défini dans la règle de style commune à `h1` et `h2`, qui définit le style général de la bordure (couleur, style), et en partie défini dans les règles de style spécifiques à `h1` et `h2`, qui définissent la largeur de la bordure. dans le règle propre à `h1`, on définit la largeur de la bordure à `2px` : la largeur de la bordure va donc surcharger la valeur de `1px` définie dans la règle commune, et la bordure du titre de niveau 1 sera donc de `2px`.

Il est tout fais possible d'écrire dans une règle - plus générale - la plupart des propriétés communes à plusieurs éléments, et d'écrire dans des règles plus spécifiques les propriétés qui diffèrent entre ces éléments. C'est une bonne pratique pour éviter les redondances dans le code CSS et pour faciliter la maintenance du code.
...
Cette organisation correspond à une grille CSS avec 3 colonnes et 2 lignes, où la première ligne contient un élément qui s'étend sur les 3 colonnes, la deuxième ligne contient 3 éléments (un dans chaque colonne), et la troisième ligne contient un élément qui s'étend sur les 3 colonnes.
...
La correction des deux sujets (qui se ressemblaient énormement) est disponible dans les fichiers suivants :
